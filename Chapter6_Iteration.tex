...
\begin{verbatim}
	int a = 5;  
	a = a + 1;         /* a + 1 results in 6. 
								and 6 is assigned to a	*/
\end{verbatim}
%
Assignment has the lowest precedent. 
This means that all other operations in the expression are evaluated before the assignment is performed. 
In the first line code {\tt a} is initialized with a value of 5. In the second line, 
all operation to the right of the assignment operator ($=$), are evaluated before the assignment is executed.
In the right side we see {\tt a + 1} which results in the value 6. The value of 6 is then assigned to {\tt a}.
